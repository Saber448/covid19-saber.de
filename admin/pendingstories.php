...
<title>Admin  - Pending stories</title>
...
$sql ="select count(*) as total from stories where statut='pending'";
     $result = $db->query($sql);
...
$records = mysqli_query($db,"select *, DATE_FORMAT(date,'%d/%m/%Y') AS date from stories where statut='pending' order by date desc LIMIT $premier, $perPage" ); 
...
?>
          ...
          <div class="col-lg-12 text-center">
			...
			<a  href="pendingstories.php?page=<?php echo $page; ?>" class="paginationlinkcss <?php echo ($currentPage == $page) ? "active" : "" ?>"><?php echo $page ?></a>
			    <?php endfor ?>
            </div>
          </div>
        </div>
